...
def fault_terminal_chart(request):
    ...
    cursor = sql_server_connect(False)
    cursor.execute("SELECT DISTINCT errname,COUNT(errname) as number FROM DataErrNow where facility_type=1 GROUP BY errname") 
    data_json = json.dumps(cursor.fetchall())
    return JsonResponse(data_json,safe=False)
...
def fault_lamp_chart(request):
    ...
    cursor = sql_server_connect(False)
    cursor.execute("SELECT DISTINCT errname,COUNT(errname) as number FROM DataErrNow where facility_type=6 GROUP BY errname") 
    data_json = json.dumps(cursor.fetchall())
    return JsonResponse(data_json,safe=False)
...
def fault_factory_chart(request):
    ...
    cursor = sql_server_connect(False)
    cursor.execute("SELECT DISTINCT TOP 20 facname,COUNT(facname) as number FROM DataFactory GROUP BY facname") 
    data_json = json.dumps(cursor.fetchall())
    return JsonResponse(data_json,safe=False)
...
def fault_terminal_table(request):
    ...
    cursor = sql_server_connect(False)
    cursor.execute("SELECT errid,errname,tmlid,tmlname,dtremove,phyid,errcount FROM DataErrNow where facility_type=1") 
    data_json = json.dumps(cursor.fetchall())
    return JsonResponse(data_json,safe=False)
...
def fault_lamp_table(request):
    ...
    cursor = sql_server_connect(False)
    cursor.execute("SELECT errid,errname,tmlid,tmlname,dtremove,phyid,errcount FROM DataErrNow where facility_type=6") 
    data_json = json.dumps(cursor.fetchall())
    return JsonResponse(data_json,safe=False)
